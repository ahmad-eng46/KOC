-- ═══════════════════════════════════════════════════════════════
-- APPLY EVERYTHING OUTSTANDING — 0060 through 0066
--
-- Paste this WHOLE file into the Supabase SQL editor and run it once.
--
-- Safe to run whatever state the database is in. Every migration in here is
-- re-runnable end to end, so applying one that already landed changes nothing.
-- If you are unsure what has been applied, this is the file to run: it is
-- cheaper than finding out.
--
--   0060  soft delete actually works (SELECT policies were refusing the write)
--   0061  staff reach everything except deleting, escalating and cost prices
--   0062  deletion approvals: optional reason, snapshots, one registry
--   0063  stored page access stops shadowing the role defaults
--         ← this is the one that returns Invoices to the staff sidebar
--   0064  a blank SKU is NULL, so a second one can be saved
--   0065  product_identity — product names readable at any role
--   0066  invoice lines carry their own name, sku and unit, plus backfill
--
-- Nothing here is destructive. It adds tables, views, functions and columns,
-- widens policies, and updates only columns it created itself. The one
-- deletion is 0063 removing user_page_access rows that merely restate the role
-- default — rows that carried no information.
--
-- BEFORE: to see 0066's backfill numbers without writing anything, run
--         supabase/tests/preview_0066_backfill.sql.
-- AFTER:  run supabase/tests/status.sql — 24 rows, all should read true.
--
-- Click into the editor and make sure NOTHING is selected before you press
-- Run. The editor runs your selection when there is one, and a fragment of
-- this will fail in confusing places.
--
-- The Results pane shows only the last statement, so success looks like
-- "Success. No rows returned". That is correct — these are DDL.
-- ═══════════════════════════════════════════════════════════════
...
-- ###########################################################
-- ##  Tell PostgREST about the new views and columns
-- ###########################################################
...
-- cache" or "column ... does not exist", even though it is right there.
